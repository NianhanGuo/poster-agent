...
export function ReferencePanel() {
  const { reference, setReference, setReferenceImage } = usePosterStore();
    ...
    <div className="p-3 space-y-4">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          ...
          <span className="font-mono text-[9px] text-zinc-500">{reference.strength}</span>
        </div>
        <input
          type="range"
          min={0}
      ...
      {!reference.imageUrl && (
        <p className="font-mono text-[8px] text-zinc-700 text-center pt-1">
          Reference metadata is passed to AI even without an image
        </p>
      )}
    </div>
  );
